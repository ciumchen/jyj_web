...
class sysitem_api_search_searchItems{
    ...
    public function getParams()
    {
        $return['params'] = array(
          ...
          //  'grade_price' => ['type'=>'string','valid'=>'','description'=>'等级折扣','example'=>'','default'=>''],
    ...
    public function getList($params)
    {
		...
		if($params['use_platform']){
			$gradeInfo = app::get('systrade')->rpcCall('user.grade.basicinfo', ['user_id'=>userAuth::id()]);
		}
        if( $itemStore || $promotionTag || $giftData)
        {
            foreach ($data['list'] as $key => &$value)
            {
				$gradePrice = json_decode($value['grade_price'],1); //@zm 等级折扣
				if($gradePrice[$gradeInfo['grade_id']]){
					$value['dis_count'] = $gradePrice[$gradeInfo['grade_id']];
				}else{
					$value['dis_count'] = $gradeInfo['dis_count'];
				}
                if( $itemStore )
                {
