...
class sysitem_finder_item{
    ...
    public function detail_basic($id)
    {
		...
		$pagedata['memberLv'] = $memberLevel->getList();
		foreach($pagedata['memberLv'] as $k=>$v){
			$memberLv[$v['grade_id']] = $v['grade_name'];
		}
		foreach($pagedata['sku'] as $k=>&$v){
			$grade_price= json_decode($v['grade_price'],1);
			$step_price= json_decode($v['step_price'],1);
			$v['step_price'] = urlencode($v['step_price']);
			if(!empty($grade_price)){
				foreach($grade_price as $gk=>$gv){
					$newgrade_price['grade_price'][$gk]['value'] = $gv?$gv:'等级无折扣';
					$newgrade_price['grade_price'][$gk]['grade_name'] = $memberLv[$gk];
				}
				$v['grade_price'] = $newgrade_price['grade_price'];
			}else{
				$v['grade_price'] = '无';
			}	
			if(!empty($step_price)){
				$v['arr_step_price'] = $step_price;
			}else{
				$v['arr_step_price'] = '无';
			}
		}
