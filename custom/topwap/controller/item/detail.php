...
class topwap_ctl_item_detail extends topwap_controller {
    ...
    public function index()
    {
		...
		$this->gradeInfo = app::get('systrade')->rpcCall('user.grade.basicinfo', ['user_id'=>$pagedata['user_id']]);
        $pagedata['image_default_id'] = $this->__setting();
        $params['item_id'] = $itemId;
        $params['fields'] = "*,item_desc.wap_desc,item_count,item_store,item_status,sku,item_nature,spec_index,grade_price";
		...
		$detailData['grade_price'] = json_decode($detailData['grade_price'],1);
		if($detailData['grade_price'][$this->gradeInfo['grade_id']]){
			$detailData['grade_dis_count'] = $detailData['grade_price'][$this->gradeInfo['grade_id']];
		}else{
			$detailData['grade_dis_count'] = $this->gradeInfo['dis_count'];
		}
        ...
        $pagedata['item'] = $this->setItemData($detailData);
        ...
        $tpl = 'topwap/item/detail/index.html';

        return $this->page($tpl, $pagedata);
    }
    ...
    public function selItem()
    {
        ...
        $tpl = 'topwap/item/detail/item_sel.html';

        return $this->page($tpl, $pagedata);
    }

    private function setItemData($detailData)
    {
        $stepPirceArr = [];
        if (! empty($detailData['step_price'])) {
            $detailData['step_price'] = json_decode($detailData['step_price'], true);
            $stepPirceArr[] = $detailData['step_price'];
        }

        $selSku = [];
        if ($detailData['spec']) {
            foreach($detailData['spec_desc'][1] as $spec) {
                $selSku[$spec['spec_value_id']] = [];
            }
            foreach($detailData['sku'] as $key => $val) {
                
                if (! empty($val['step_price'])) {
                    // $detailData['sku'][$key]['step_price'] = json_decode($val['step_price'], true);
                    $val['step_price'] = json_decode($val['step_price'], true);
                }

                if ($val['step_price']) {
                    $stepPirceArr[] = $val['step_price'];
                }

                if ($detailData['step_price'] && ! $val['step_price']) {
                    $val['step_price'] = $detailData['step_price'];
                }
                
                if (count($val['spec_desc']['spec_value_id']) > 1){
                    $val['spec_info_2'] = explode('、', $val['spec_info'])[1];
                } else {
                    $val['spec_info_2'] = $val['spec_info'];
                }

                $selSku[$val['spec_desc']['spec_value_id'][1]][] = $val;

            }
            ksort($selSku);
            $detailData['spec_desc'][1] = array_values($detailData['spec_desc'][1]);
        }else{
            if (! $detailData['sku']) {
                exit();
            }
        ...
        if ($stepPirceArr) {
            $tempArr = [];
            foreach($stepPirceArr as $key => $val) {
                $lowArr = [];
                foreach($val as $k => $v) {
                    $lowArr[] = $v['p'];
                }
                sort($lowArr);
                if (isset($lowArr[0]) && ! isset($tempArr[$lowArr[0]])) {
                    $tempArr[$lowArr[0]] = $val;
                }
            }
            ksort($tempArr);
            $tempArr = array_values($tempArr);
            if (isset($tempArr[0])) {
                $detailData['def_step_price'] = $tempArr[0];
            }
        }else{
            $detailData['def_step_price'] = '';
        }

        if ($detailData['def_step_price']) {
            foreach($detailData['def_step_price'] as $k => $v) {
                $arr = explode('.', strval(sprintf("%.2f", $v['p'])));
                $str =  '¥<span>' . $arr[0] . '</span>.' . $arr[1] . '';
                $detailData['def_step_price'][$k]['p_desc'] = $str;
            }
        }

        $detailData['sel_sku'] = $selSku;
    ...
    private function __getSpec($spec, $sku)
    {
		...
		$this->gradeInfo = app::get('systrade')->rpcCall('user.grade.basicinfo',['user_id'=>userAuth::id()]);
        foreach( $sku as $row )
        {
            ...
            if( $key )
            {
				...
				//@zm 添加等级价格
				$result['specSku'][$key]['grade_price'] = json_decode($row['grade_price'],1);
				if($result['specSku'][$key]['grade_price'][$this->gradeInfo['grade_id']]){
					if($activityPrice>0){
						$result['specSku'][$key]['grade_price'] = ecmath::number_multiple(array($result['specSku'][$key]['grade_price'][$this->gradeInfo['grade_id']],$activityPrice));	
					}else{
						$result['specSku'][$key]['grade_price'] = ecmath::number_multiple(array($result['specSku'][$key]['grade_price'][$this->gradeInfo['grade_id']],$row['price']));
					}
				}else{
					if($activityPrice>0){
						$result['specSku'][$key]['grade_price'] = ecmath::number_multiple(array($this->gradeInfo['dis_count'],$activityPrice));	
					}else{
						$result['specSku'][$key]['grade_price'] = ecmath::number_multiple(array($this->gradeInfo['dis_count'],$row['price']));
					}
				}
