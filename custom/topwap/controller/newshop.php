...
class topwap_ctl_newshop extends topwap_controller{
    ...
    public function index()
    {
        ...
        //获取店铺详情 百度地图使用
        $shopInfo= app::get('topwap')->rpcCall('shop.get',['shop_id'=>$shopId]);
        $area = explode(':', $shopInfo['shop_area']);
        $shopInfo['area'] = $area[1];
        $shopInfo['shop_area'] = $area[0];
...
// echo '<pre>';var_dump($this->gradeInfo);				
// echo '<pre>';var_dump($itemsList);				
		foreach($itemsList['list'] as $k=>$item){
						...
						if($item['grade_price'][$this->gradeInfo['grade_id']]){
							$itemsList['list'][$k]['grade_dis_count'] = $item['grade_price'][$this->gradeInfo['grade_id']];
						}else{
							$itemsList['list'][$k]['grade_dis_count'] = $this->gradeInfo['dis_count'];
						}
		}
