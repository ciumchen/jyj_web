...
class topwap_ctl_passport extends topwap_controller{
    ...
    public function checkUname()
    {
        ...
        {
            if(!preg_match("/^1[345789]{1}[0-9]{9}$/", $uname))
            {
        ...
        if($userData)
        {
            if ($userData['check_status'] == 1) {
                $msg = app::get('topwap')->_("该用户名或手机号已经使用");
                return $this->splash('error','',$msg);
            } elseif ($userData['check_status'] == 0) {
                $msg = app::get('topwap')->_("该用户名或手机号已经使用且在审核中");
                return $this->splash('error','',$msg);
            }
        }
    ...
    public function doRegister()
    {
		...
		$data['pam_user']['account'] = $data['uname'];//@zm 
        $codyKey = $data['key'];
        ...
        $validator = validator::make(
            ...
            ['loginAccount'=>'请输入手机号!','password' => '密码长度不能小于6位!|密码长度不能大于20位!|输入的密码不一致!','password_confirmation'=>'确认密码不能为空!']
        );
        if ($validator->fails())
        {
        ...
        try
        {
			...
			$userdata = $data['shop'];
			if($userdata['shoparea']){
				$postarea = input::get('shop');
				$area = app::get('topwap')->rpcCall('logistics.area',array('area'=>$postarea['shoparea']));				
				$areaId =  rtrim(str_replace(",","/", $postarea['shoparea']),'/');
				$userdata['shoparea'] = $area . ':' . $areaId;
			}
			app::get('sysuser')->model('user')->update($userdata,array('user_id'=>$userId));
    ...
    public function verifyUsername()
    {
        ...
        if($postdata['username'])
        {
            ...
            {
                $randomId = str_random(32);
                $_SESSION['topapi'.$data['mobile']] = $randomId;
                $data['type'] = "forgot";
                $pagedata['data'] = $data;
                return view::make('topwap/passport/verify_vcode.html',$pagedata);
            }
    ...
    public function sendVcode()
    {
        ...
        $uname = $postdata['uname'];//@zm 增加手机号验证，生成session topapi
		if(!$uname){
			return $this->splash('error','','请输入手机号');
		}
        $userData = userAuth::getAccountInfo($uname);
        
        if($userData)
        {
            $userAccount = app::get('sysuser')->model('account')->getRow('check_status', ['user_id' => $userData['user_id']]);
            // var_dump($userAccount);
            if ($userAccount['check_status'] == 1) {
                $msg = app::get('topwap')->_("该用户名或手机号已经使用");
                return $this->splash('error','',$msg);
            } elseif ($userAccount['check_status'] == 0) {
                $msg = app::get('topwap')->_("该用户名或手机号已经使用且在审核中");
                return $this->splash('error','',$msg);
            }
        }
	...
	public function ajaxCheckLogin(){
		// if(!preg_match('/passport-gologin|passport-goregister|register-license|passport-getwxopenid/i',$_POST['url']) && !preg_match('/passport-gofindpwd/i',$_POST['url'])){
			// if (!userAuth::check())
			// {	
				// return response::json(array('err_code'=>'1'));
	...
	public function getwxopenid(){
				...
				$wxcode = $_GET['code'];
        if($wxcode){
            $wxpayConf = app::get('ectools')->getConf('ectools_payment_plugin_wxpayxcx');
            $wxpayConf = unserialize($wxpayConf);
           $appId = $wxpayConf['setting']['appId'];
           $appSecret = $wxpayConf['setting']['Appsecret'];
            $url = 'https://api.weixin.qq.com/sns/jscode2session?appid='.$appId.'&secret='.$appSecret.'&js_code='.$wxcode.'&grant_type=authorization_code';
            $response = kernel::single('base_httpclient')->get($url);
			$res = json_decode($response,1);
			$_SESSION['xcxopenid'] = $res['openid'];
			setCookie('xcxopenid',$res['openid']);
            echo $response;
        }
	}
}
