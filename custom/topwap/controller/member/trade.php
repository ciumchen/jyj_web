...
class topwap_ctl_member_trade extends topwap_ctl_member {
        ...
        foreach ($trade['orders'] as $k => $v) {
            if($v['cat_id'] && $v['status'] =='TRADE_FINISHED' && $v['end_time'])
            {
                ...
                $trade['orders'][$k]['changing_enabled'] = $aftersalesEnabled['changing_enabled'];
							...
							$trade['orders'][$k]['allowAfterSale'] = true;
				//申请售后数量
				$aftersalesInfo = app::get('base')->database()->executeQuery('SELECT SUM(num) num FROM sysaftersales_aftersales WHERE oid='.$v['oid'])->fetch();

				if($aftersalesInfo['num']>0 && $aftersalesInfo['num']>=$v['num']){
					 $trade['orders'][$k]['allowAfterSale'] = false;
				}
        }
        // 订单配送方式
        $shippingName = array(
    ...
    private function __getTrade($postdata)
    {
        ...
        $params ['page_size'] = intval($this->limit);
        $params ['order_by'] = 'created_time desc';
        $params ['fields'] = 'order.grade_dis_count,order.step_price,tid,shop_id,user_id,status,payment,points_fee,total_fee,cancel_status,itemnum,post_fee,payed_fee,receiver_name,created_time,receiver_mobile,discount_fee,need_invoice,adjust_fee,order.title,order.price,order.num,order.pic_path,order.tid,order.oid,order.aftersales_status,order.end_time,buyer_rate,order.complaints_status,order.item_id,order.cat_id,order.shop_id,order.status,activity,pay_type,order.spec_nature_info,order.gift_data';

        $tradelist = app::get('topwap')->rpcCall('trade.get.list', $params);

        $count = $tradelist ['count'];
        $tradelist = $tradelist ['list'];
// echo '<pre>';var_dump($tradelist);
        foreach ($tradelist as $key => $row)
        {
            ...
            foreach ($row ['order'] as $orderListData)
            {
				                ...
				                if( isset($orderListData['aftersales_status']) && $orderListData['aftersales_status'] )
                {
                    ...
                    $tradelist[$key]['aftersales_type'] = $afterSelf['aftersales_type'];
                }
                ...
                if($orderListData['cat_id'] && $orderListData['status'] =='TRADE_FINISHED' && $orderListData['end_time'])
                {
                    ...
                    $tradelist[$key]['refund_enabled'] = $aftersalesEnabled['refund_enabled'];
                    $tradelist[$key]['changing_enabled'] = $aftersalesEnabled['changing_enabled'];
                    $tradelist[$key]['oid'] = $orderListData['oid'];
                }
            }
