...
class topwap_ctl_member_aftersales extends topwap_ctl_member {
    ...
    public function aftersalesApply()
    {
        ...
        // 获取商品信息
        $filter['oid'] = $oid;
        $filter['fields'] = 'user_id,item_id,cat_id,end_time,bn,title,price,num,pic_path,spec_nature_info,gift_data';
        ...
        if(!$orderInfo['refund_enabled'] && $orderInfo['changing_enabled']){
            $pagedata['title'] = "申请换货";
				...
				//可以申请售后的数量
				$aftersalesInfo = app::get('base')->database()->executeQuery('SELECT SUM(num) num FROM sysaftersales_aftersales WHERE status<>"3" AND status<>"6" AND oid='.$oid)->fetch();
				if($aftersalesInfo['num']>0){
					$orderInfo['max'] = $orderInfo['num']-$aftersalesInfo['num'];
				}else{
					$orderInfo['max'] = $orderInfo['num'];
				}
        $pagedata['tid'] = $tid;
        $pagedata['oid'] = $oid;
    ...
    public function commitAftersalesApply()
    {
        ...
        $validator = validator::make(
            ['reason'=>$postdata['reason'],'description'=>$postdata['description'],'oid'=>$postdata['oid'],'aftersales_type'=>$postdata['aftersales_type'],'num'=>$postdata['num']],
            ['reason'=>'required','description'=>'|max:300','oid'=>'numeric','aftersales_type'=>'required','num'=>'required|numeric|min:1'],
            ['reason'=>'售后申请理由必填!','description'=>'描述不能大于300','oid'=>'订单格式不对','aftersales_type'=>'请选择售后类型','num'=>'请输入正确的数量|请输入正确的数量|售后数量必须大于1']
        );
        if ($validator->fails())
        {
