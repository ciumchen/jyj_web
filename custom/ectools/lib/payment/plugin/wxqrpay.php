...
final class ectools_payment_plugin_wxqrpay extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    public function __construct($app){
        ...
        $this->refund_url_check = "https://api.mch.weixin.qq.com/pay/refundquery";
    ...
    protected function get_html(){
        ...
        $strHtml .=
            '<div id="qrcode">
            <sapn class="dialog-btn-close"></sapn>
                ...
                xmlhttp.onreadystatechange=function()
                {
                    if (xmlhttp.readyState==4 && xmlhttp.status==200)
                    {
                        if(xmlhttp.responseText=="succ")
                        {
                            ...
                            time_js_item = setTimeout("check_trade()",500);
                        }
                    }
                }
            }
    ...
    public function dorefund($payment)
    {
        ...
        if(strtoupper($result['return_code']) == 'SUCCESS')
        {
            ...
            for($i=0;$i<10;$i++){
                ...
                $parameter_refund['sign'] = $this->getSign($parameter_refund, $key);
                $xml_refund      = $this->arrayToXml($parameter_refund);
                $response_refund = client::post($this->refund_url_check, ['body' => $xml_refund])->getBody();
                $result_refund   = $this->xmlToArray($response_refund);
                logger::info('微信扫码支付退款查询返回的信息：'.var_export($result_refund,1));

                if(strtoupper($result_refund['result_code']) == 'SUCCESS'){
                    break;
                }
                sleep(3);//停3秒继续查
            }
