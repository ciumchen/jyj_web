...
class topc_ctl_shopcenter extends topc_controller
{
            ...
            if( $activityItemList )
            {
                ...
                foreach( $items as &$row )
                {
                    foreach( $row as &$item )
                    {
						...
						$item['grade_price'] = json_decode($item['grade_price'],1);	
						if($item['grade_price'][$this->gradeInfo['grade_id']]){
							$item['grade_dis_count'] = $item['grade_price'][$this->gradeInfo['grade_id']];
						}else{
							$item['grade_dis_count'] = $this->gradeInfo['dis_count'];
						}
                    }
                }
            }
    ...
    public function search()
    {
        ...
        if($itemsList['list'])
        {
			...
			foreach($itemsList['list'] as $k=>$item){
						$itemsList['list'][$item['item_id']]['grade_price'] = json_decode($item['grade_price'],1);	
						if($itemsList['list'][$item['item_id']]['grade_price'][$this->gradeInfo['grade_id']]){
							$itemsList['list'][$item['item_id']]['grade_dis_count'] = $itemsList['list'][$item['item_id']]['grade_price'][$this->gradeInfo['grade_id']];
						}else{
							$itemsList['list'][$item['item_id']]['grade_dis_count'] = $this->gradeInfo['dis_count'];
						}	
			}
        }
