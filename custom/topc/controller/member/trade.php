    ...
    public function tradeList()
    {
        ...
        foreach( $tradelist as $key=>&$row)
        {
            ...
            foreach( $row['order'] as $k=>$orderListData )
            {
				...
				$tradelist[$key]['order'][$k]['allowAfterSale'] = true;
				//申请售后数量
				$aftersalesInfo = app::get('base')->database()->executeQuery('SELECT SUM(num) num FROM sysaftersales_aftersales WHERE oid='.$orderListData['oid'])->fetch();

				if($aftersalesInfo['num']>0 && $aftersalesInfo['num']>=$orderListData['num']){
					 $tradelist[$key]['order'][$k]['allowAfterSale'] = false;
				}
            }
            // 获取店铺子域名
