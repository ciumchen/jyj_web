...
class topc_ctl_member_aftersales extends topc_ctl_member{
    ...
    public function aftersalesApply()
    {
        ...
        try
        {
            ...
            foreach($pagedata['tradeInfo']['orders'] as $k => $order)
            {
				...
				//可以申请售后的数量
				$aftersalesInfo = app::get('base')->database()->executeQuery('SELECT SUM(num) num FROM sysaftersales_aftersales WHERE  status<>"3" AND status<>"6" AND oid='.$order['oid'])->fetch();
				if($aftersalesInfo['num']>0){
					$pagedata['tradeInfo']['orders'][$k]['max'] = $order['num']-$aftersalesInfo['num'];
				}else{
					$pagedata['tradeInfo']['orders'][$k]['max'] = $order['num'];
				}
    ...
    public function commitAftersalesApply()
    {
        ...
        $validator = validator::make(
            ['reason'=>$postdata['reason'],'description'=>$postdata['description'],'oid'=>$postdata['oid'],'num'=>$postdata['num']],
            ['reason'=>'required','description'=>'|max:300','oid'=>'numeric','num'=>'required|numeric|min:1'],
            ['reason'=>'售后申请理由必填!','description'=>'描述不能大于300','oid'=>'订单格式不对','num'=>'售后申请数量填写错误|售后申请数量填写错误|售后数量必须大于1']
        );
        if ($validator->fails())
        {
        ...
        try
        {
            $aftersalesApplyRpcRequestParams = input::get();
            $aftersalesApplyRpcRequestParams['user_id'] = userAuth::id();
            $result = app::get('topc')->rpcCall('aftersales.apply', $aftersalesApplyRpcRequestParams);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
