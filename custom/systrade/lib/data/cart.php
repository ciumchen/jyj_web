...
class systrade_data_cart {
    ...
    private function __preShopCartInfo($shopCartData, $needInvalid, $platform)
    {
        ...
        if($itemIds && $skuIds)
        {
            $itemRows = 'item_id,cat_id,title,weight,image_default_id,sub_stock,violation,disabled,dlytmpl_id,step_price,grade_price,mini_quantity,unit';
            $skuRows = 'sku_id,bn,item_id,spec_info,price,weight,status,step_price,grade_price,spec_key';
        ...
        foreach( $shopCartData as $row )
        {
            ...
            $row['grade_dis_count'] = $this->gradeInfo['dis_count']; //等级折扣
