   ...
   /**
    ...
    protected function genId($userId, $isTid=true )
    {
        if(!defined('STRESS_TESTING'))
        {
            ...
            $redisId = redis::scene('tradeOrderId')->hincrby(date('Ymd'), $this->minute, rand(1,9));
    ...
    private function __orderItemData($tradeParams, $cartItem, $shopId)
    {
		...
		$step_price = '';
		if($cartItem['step_price_cur']){
			$step_price = $cartItem['step_price'][$cartItem['step_price_cur'][1]]; //记录sku阶梯优惠
		}
        $orderData = [
            'oid'              => $oid,
            ...
            'grade_dis_count'  => $cartItem['grade_dis_count'],
        ];
