...
class systrade_cart_object_item implements systrade_interface_cart_object{
    ...
    public function processCartObject($row,$itemsData,$skusData,$itemActivityList)
    {
        ...
        $objectData['mini_quantity'] = $itemsData[$itemId]['mini_quantity'];
        $objectData['unit'] = $itemsData[$itemId]['unit'];
		if($skusData[$skuId]['step_price']){
		        $objectData['step_price'] = json_decode($skusData[$skuId]['step_price'],1);//@zm	
		}elseif($itemsData[$row['item_id']]['step_price']){
			 $objectData['step_price'] = json_decode($itemsData[$row['item_id']]['step_price'],1);//@zm	
		}	
		if($skusData[$skuId]['grade_price']){
		        $objectData['grade_price'] = json_decode($skusData[$skuId]['grade_price'],1);//@zm	
		}elseif($itemsData[$row['item_id']]['grade_price']){
			 $objectData['grade_price'] = json_decode($itemsData[$row['item_id']]['grade_price'],1);//@zm	
		}

        $objectData['step_price_cur'] = 'none';//@zm
        $objectData['grade_dis_count'] = 0;//@zm
        $objectData['bn'] = $skusData[$skuId]['bn'];
        //分类类型
        $objectData['spec_key'] = $skusData[$skuId]['spec_key'];
		...
		$objectData['price']['oldprice'] = $skusData[$skuId]['price'];//初始价格
		$objectData['price']['grade_price_pre'] = $skusData[$skuId]['price'];//等级价优惠前价格
		//@zm 阶梯价，等级折扣
        if($activityDetail && ((!$activityDetail['sku_id'] && !$activityDetail['sku_activity_price']) || $activityDetail['sku_activity_price'][$skuId]))
        {
			...
			if($objectData['step_price']){  //@zm设置了阶梯价，则判断数量是不是在阶梯价范围内。
				foreach($objectData['step_price'] as $k=>$v){
					if($row['quantity']>=$v['f'] && $row['quantity']<=$v['e']){
						$objectData['price']['price'] = $v['p'];
						$objectData['step_price_cur'] = 'k'.$k;
						break;
					}
				}
			}
			$objectData['price']['grade_price_pre'] = $objectData['price']['price'];//等级价优惠前价格

			if($objectData['grade_price'] && $objectData['grade_price'][$row['grade_id']]){//@zm商品设置了等级价用商品等级折扣，没有设置则使用平台等级折扣
				$objectData['price']['price'] = ecmath::number_multiple(array($objectData['price']['price'],$objectData['grade_price'][$row['grade_id']]));
				$objectData['grade_dis_count'] = ecmath::number_multiple(array($row['quantity'],$objectData['price']['grade_price_pre']-$objectData['price']['price']));
			}elseif($row['grade_dis_count']){
				$objectData['price']['price'] = ecmath::number_multiple(array($objectData['price']['price'],$row['grade_dis_count']));
				$objectData['grade_dis_count'] = ecmath::number_multiple(array($row['quantity'],$objectData['price']['grade_price_pre']-$objectData['price']['price']));
			}
        ...
        else
        {
			...
			$objectData['price']['grade_price_pre'] = $skusData[$skuId]['price'];//等级价优惠前价格
			if($objectData['grade_price'] && $objectData['grade_price'][$row['grade_id']]){//@zm商品设置了等级价用商品等级折扣，没有设置则使用平台等级折扣
				$skusData[$skuId]['price'] = ecmath::number_multiple(array($skusData[$skuId]['price'],$objectData['grade_price'][$row['grade_id']]));
				$objectData['grade_dis_count'] = ecmath::number_multiple(array($row['quantity'],$objectData['price']['grade_price_pre']-$skusData[$skuId]['price']));
			}elseif($row['grade_dis_count']){
				$skusData[$skuId]['price'] = ecmath::number_multiple(array($skusData[$skuId]['price'],$row['grade_dis_count']));
				$objectData['grade_dis_count'] = ecmath::number_multiple(array($row['quantity'],$objectData['price']['grade_price_pre']-$skusData[$skuId]['price']));
			}
