...
class sysaftersales_api_list {
   ...
   public function getParams()
    {
        $return['params'] = array(
            'user_id' => ['type'=>'string','valid'=>'', 'description'=>'会员ID,user_id和shop_id必填一个'],
            'shop_id' => ['type'=>'int','valid'=>'int', 'description'=>'店铺ID,user_id和shop_id必填一个'],
            'tid' => ['type'=>'string','valid'=>'', 'description'=>'订单编号'],
            'oid' => ['type'=>'string','valid'=>'', 'description'=>'订单编号'],
            'title' => ['type'=>'string','valid'=>'', 'description'=>'商品名称'],
            'created_time' => ['type'=>'json_encode','valid'=>'', 'description'=>'申请时间范围'],
            'aftersales_bn' => ['type'=>'int','valid'=>'numeric', 'description'=>'退换货编号'],
            'aftersales_type' => ['type'=>'string','valid'=>'', 'description'=>'退换货类型'],
            'progress' => ['type'=>'string','valid'=>'', 'description'=>'退换货处理进度'],
            'page_no' => ['type'=>'int','valid'=>'numeric', 'description'=>'分页当前页数,默认为1'],
            'page_size' => ['type'=>'int','valid'=>'numeric', 'description'=>'每页数据条数,默认10条'],
    ...
    { 
        ...
        $filterFields = ['title','user_id','shop_id','tid','oid','created_time','aftersales_bn','aftersales_type','progress'];
        foreach( $filterFields as $value )
        {
            if( isset($params[$value]) )
            {
                if( $value == 'title' )
                {
                    $filter[$value.'|has'] = $params['title'];
                }
                elseif( $value == 'progress' || $value == 'user_id' )
                {
                    $filter[$value] = explode(',',$params[$value]);
        ...
        if( $filter['user_id'] )
        {
            foreach( (array)$filter['user_id'] as $userId )
            {
                if( $userId && !is_numeric($userId) )
                {
                    throw new \LogicException(app::get('sysaftersales')->_('会员ID必须为int'));
                }
            }
        }
        ...
        if($filter['created_time'])
        {
            ...
            if(array_filter($times))
            {
                ...
                $filter['created_time|lthan'] = strtotime($times['1'])+86400;
