...
class sysaftersales_apply {
    ...
    public function create($data)
    {
        $tradeFiltr = array(
            'tid' => $data['tid'],
            'oid' => $data['oid'],
            'fields' => 'orders.item_id,orders.cat_id,orders.sku_id,tid,user_id,shop_id,status,orders.oid,orders.user_id,orders.sku_id,orders.num,orders.sendnum,orders.title,orders.gift_data',
        );
    ...
    {
        $this->__checkApplyBySystemConf($tradeInfo, $data);
        $aftersalesInfo = app::get('base')->database()->executeQuery('SELECT SUM(num) num FROM sysaftersales_aftersales WHERE status<>"3" AND status<>"6" AND oid='.$data['oid'])->fetch();
        // 判断是否通过平台申诉
        $flag = false;
		...
		$afterSaleNum = $tradeInfo['orders'][0]['num']-$aftersalesInfo['num'];		
		if($data['num']>$afterSaleNum){
			 throw new \LogicException(app::get('sysaftersales')->_('申请售后数量最大为：'.$afterSaleNum));
		}

        if( $afterSaleNum<=0 && !$flag)
        {
    ...
    private function __checkApplyBySystemConf($tradeInfo, $data)
    {
        //这里判断系统是否可以退换货
        //售后服务类型(ONLY_REFUND:只退款，REFUND_GOODS:退货退款，EXCHANGING_GOODS:换货) 默认为只退款
        $applyAftersalesType = $data['aftersales_type'];
        $cat_id = $tradeInfo['orders'][0]['cat_id'];
        $aftersalesSystemConf = kernel::single('sysaftersales_setting')->getAftersalesSettingByCatId($params['cat_id']);
        if($applyAftersalesType == 'ONLY_REFUND' || $applyAftersalesType == 'REFUND_GOODS' )
        {
            if(is_null($aftersalesSystemConf['refund_active']))
                throw new LogicException(
                    app::get('sysaftersales')->_('暂时无法退货，请联系客服')
                );
        }elseif($applyAftersalesType == 'EXCHANGING_GOODS'){
            if(is_null($aftersalesSystemConf['changing_active']))
                throw new LogicException(
                    app::get('sysaftersales')->_('暂时无法换货，请联系客服')
                );
        }
        return true;
    }
