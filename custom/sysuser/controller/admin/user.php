...
class sysuser_ctl_admin_user extends desktop_controller {
    ...
    public function index()
    {
        ...
        if ($view == 0){
            $actions = [
                ...
                array(
                    'label'=>app::get('sysuser')->_('添加会员'),
                    'target'=>'dialog::{ title:\''.app::get('sysuser')->_('添加会员').'\', width:850, height:600}',
                    'href'=>'?app=sysuser&ctl=admin_user&act=edit',
                ),		
            ];
        }
    ...
    {
		$pagedata['grade'] = app::get('sysuser')->model('user_grade')->getList();
        return $this->page('sysuser/admin/user/add.html', $pagedata);
    }
	public function saveAdd(){
        ...
        $this->begin("?app=sysuser&ctl=admin_user&act=index");
        try
        {
            ...
            if(!$userId){
                    throw new Exception(app::get('sysuser')->_('会员添加失败！'));
            }
			//保存公司信息数据
			$company_data = ['company_name'=>$data['company_name'],'addr'=>$data['addr'],'touch_name'=>$data['touch_name'],'business_img'=>$data['business_img'],'grade_id'=>$data['grade_id']];
            $objMdlUser->update($company_data,array('user_id'=>$userId));
			$account_data = ['mobile'=>$data['mobile']];
            $objMdlaccount->update($account_data,array('user_id'=>$userId));
			$this->adminlog("添加会员：{$userInfo['account']}", 1);
        }
        catch(Exception $e)
        {
            $msg = $e->getMessage();
    ...
    public function saveCompanyInfo()
    {
			...
			$userId = $data['user_id'];
			//保存公司信息数据
			$company_data = ['company_name'=>$data['company_name'],'addr'=>$data['addr'],'touch_name'=>$data['touch_name'],'business_img'=>$data['business_img']];
            $objMdlUser->update($company_data,array('user_id'=>$userId));
			$account_data = ['mobile'=>$data['mobile']];
            $objMdlaccount->update($account_data,array('user_id'=>$userId));
            $this->adminlog("修改会员公司信息[USER_ID:{$userId}]", 1);
    ...
    public function sendSms()
    {
        ...
        {
            $data = $_POST;
			$paminfo = app::get('sysuser')->model('account')->getRow('*',array('user_id'=>$data['user_id']));
			$sendResult = kernel::single('system_messenger_sms')->send($paminfo['mobile'],'',$data['content'],$config);
        }
        catch(Exception $e)
        {
            $msg = $e->getMessage();
