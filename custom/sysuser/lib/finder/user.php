...
class sysuser_finder_user {
    ...
    public function __construct($app)
    {
        ...
        $this->detail_sms = app::get('sysuser')->_('发送短信');
        ...
        $this->detail_company = app::get('sysuser')->_('公司信息');
        $this->detail_shop = app::get('sysuser')->_('店铺信息');
    }
    ...
    public function column_editbutton(&$colList, $list)
    {
        foreach($list as $k=>$row)
        {
            if (intval($_GET['view'] == 0)) {
                ...
                $showTitle = intval($_GET['view'] == 1) ? '审核' : '查看';
                $title = app::get('sysuser')->_($showTitle);
                $url = url::route('shopadmin', ['app'=>'sysuser','act'=>'index','ctl'=>'admin_check','finder_id'=>$_GET['_finder']['finder_id'],'id'=>$row['user_id'],'finderview'=>'detail_basic','action'=>'detail','singlepage'=>'true']);
                $colList[$k] = '<a href="'.$url.'" target="_blank" title="'.$showTitle.'">'.$showTitle.'</a>';
            }
        }
    }
    ...
    public function _column_editbutton($row)
    {
        ...
        $arr_link = array(
            ...
            'finder'=>array(
				...
				'detail_sms'=>array(
                    'href'=>'javascript:void(0);',
                    'submit'=>'?'.$newu.'&finderview=detail_sms&id='.$row['user_id'].'&_finder[finder_id]='.$_GET['_finder']['finder_id'],'label'=>app::get('sysuser')->_('发送短信'),
                    'target'=>'tab',
                ),
				...
				'detail_company'=>array(
                    'href'=>'javascript:void(0);',
                    'submit'=>'?'.$newu.'&finderview=detail_company&id='.$row['user_id'].'&_finder[finder_id]='.$_GET['_finder']['finder_id'],'label'=>app::get('sysuser')->_('公司信息'),
                    'target'=>'tab',
                ),		
				'detail_shop'=>array(
                    'href'=>'javascript:void(0);',
                    'submit'=>'?'.$newu.'&finderview=detail_shop&id='.$row['user_id'].'&_finder[finder_id]='.$_GET['_finder']['finder_id'],'label'=>app::get('sysuser')->_('店铺信息'),
                    'target'=>'tab',
                ),
	...
	public function detail_sms($row)
    {
        $paminfo = app::get('sysuser')->model('account')->getRow('*',array('user_id'=>$row));
        $sysinfo['login_account'] = $paminfo['login_account'];
        $sysinfo['user_id'] = $paminfo['user_id'];
        $sysinfo['mobile'] = $paminfo['mobile'];

        $pagedata['data'] = $sysinfo;
        return view::make('sysuser/admin/user/sendsms.html', $pagedata)->render();
        //return 'ok';
    }
	...
	public function detail_company($row)
    {
        $url = '?app=sysuser&ctl=admin_user&act=editCompanyInfo&finder_id='.$_GET['_finder']['finder_id'].'&p[0]='.$row;
        $pagedata['url'] = $url;
        $sysinfo = kernel::single('sysuser_passport')->memInfo($row);
        $pagedata['data'] = $sysinfo;
        return view::make('sysuser/admin/user/detail_company.html', $pagedata)->render();
    }	
	public function detail_shop($row)
    {
        $sysinfo = app::get('sysuser')->model('user')->dump($row);
		if($sysinfo['shoparea']){
			$sysinfo['shoparea'] =  explode(":",$sysinfo['shoparea'])[0];
		}
        $pagedata['data'] = $sysinfo;
        return view::make('sysuser/admin/user/detail_shop.html', $pagedata)->render();
    }
}
