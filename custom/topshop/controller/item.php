...
class topshop_ctl_item extends topshop_controller {
    ...
    private function _formatItemData($postData)
    {
        ...
        $data['mini_quantity'] = $postData['item']['mini_quantity'] ? : 0;
        ...
        if( $sku && $data['nospec'])
        {
            foreach($sku as $key=>&$val)
            {
                ...
                $val['grade_price'] = $postData['item'][$key]['grade_price'];
                $val['step_price'] = $postData['item'][$key]['step_price'];
            }
            $data['sku'] = json_encode($sku);
        //}elseif($sku){
	        // foreach($sku as $key=>&$val)
            // {
                // $val['grade_price'] = $postData[$key]['grade_price'];
                // $val['step_price'] = $postData[$key]['step_price'];
            // }
            // $data['sku'] = json_encode($sku);		 
		}
        ...
        $data['grade_price'] = $postData['item']['grade_price'];//单品时候需要
        $data['step_price'] = $postData['item']['step_price'];//单品时候需要
        return $data;
    }
    ...
    function set_mprice(){
		$data = $_POST;
        $memberLevel = app::get('sysuser')->model('user_grade');
		if($data['prefix_value']){
			//本身已经有的等级价格则生成数据分配
			$prefix_value = json_decode($data['prefix_value'],1);
			
		} 
        foreach($memberLevel->getList() as $level){
            $level['dis_count'] = ($level['dis_count']>0 ? $level['dis_count'] : 1);
            $level['price'] = $prefix_value[$level['grade_id']];
            $pagedata['item'][$level['grade_id']] = $level;
        }
		$pagedata['prefix'] = $data['prefix'];
		$pagedata['specname'] = $data['specname'];
		return view::make('topshop/item/set_mprice.html', $pagedata);
    }    
	function set_step_price(){
		$data = $_POST;
		if($data['prefix_value']){
			//本身已经有的等级价格则生成数据分配
			$prefix_value = json_decode($data['prefix_value'],1);
		...
		$pagedata['prefix'] = $data['prefix'];
		$pagedata['item'] = $prefix_value;
		$pagedata['specname'] = $data['specname'];
		return view::make('topshop/item/set_step_price.html', $pagedata);
    }
	function check_step_price(){
		$data = input::get();
		$return = ['error'=>0,'msg'=>'','data'=>[],'current'=>0];
		if($data){
			foreach($data['from'] as $k=>$v){
				...
				if($v>=$data['end'][$k]){
					$return['error'] = 1;
					$return['current'] = $k;
					$return['msg']	 = '结束数量不能小于开始数量';
					break;
				}	
				if(!is_numeric($data['discount'][$k])){
					$return['error'] = 3;
					$return['current'] = $k;
					$return['msg']	 = '请输入正确的价格';
					break;
				}
				// if(!is_int($v) || !is_int($data['end'][$k])){
					// $return['error'] = 3;
					// $return['current'] = $k;
					// $return['msg']	 = '数量请输入正整数';
					// break;				
				// }
				if(isset($data['from'][$k-1])){
					//前边有数量，判断起始数量是不是和结束数量不相等；
					if($v<=$data['end'][$k-1]){
						$return['error'] = 2;
						$return['current'] = $k;
						$return['msg']	 = '开始数量必须大于前一个结束数量';
						break;					
					}
				}
				$return['data'][$k]['f'] = $v;
				$return['data'][$k]['e'] = $data['end'][$k];
				$return['data'][$k]['p'] = $data['discount'][$k];
			}
			$return['data'] = json_encode($return['data']);
		}
	    return response::json($return);exit;	
	}
}
