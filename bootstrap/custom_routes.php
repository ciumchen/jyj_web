...
{
    route::match(array('GET','POST'),'/', ['uses'=>'topapi_server@process', 'middleware' => ['topapi_middleware_checkfields']]);
});

route::group(array('prefix' => 'shop/topapi'), function()
{
    route::match(array('GET','POST'),'/', ['uses'=>'topmanageapi_server@process', 'middleware' => ['topapi_middleware_checkfields']]);
});
...
route::group(array('prefix' => 'wechat/'), function() {
    route::match(array('GET','POST'),'/', ['uses'=>'syswechat_server@process']);
});
    ...
    */
    route::group(array(), function() {
        ...
        */
		...
		# 验证登录状态
        route::post('passport-checkLogin.html', ['uses' => 'topc_ctl_passport@ajaxCheckLogin' ]);
    });
...
route::group(array('prefix' => 'wap', 'middleware' => ['theme_middleware_preview', 'base_middleware_machine_check', 'base_middleware_machine_hook']), function() {
    ...
    route::get('brand.html', [ 'as' => 'topwap.app.brand', 'uses' => 'topwap_ctl_brand@index']);
    ...
    route::get('item-sel.html', [ 'uses' => 'topwap_ctl_item_detail@selItem' ]);
    ...
    route::post('passport-send_vcode.html', [ 'as' => 'send.vcode', 'uses' => 'topwap_ctl_passport@sendVcode']);
		//获取微信小程序openid
    route::post('passport-getwxopenid.html', [ 'as' => 'get.openid', 'uses' => 'topwap_ctl_passport@getwxopenid']);
    //验证手机验证码
	...
	//登录验证
    route::post('passport-checkLogin.html', [ 'as' => 'checkLogin', 'uses' => 'topwap_ctl_passport@ajaxCheckLogin']); 
    ...
    route::match(array('GET', 'POST', 'PUT', 'DELETE'), 'wxpayxcx.html', ['uses' => 'topwap_ctl_wechat@wxpayxcx']); 
    ...
    route::post('cart-batch-add.html', [ 'uses' => 'topwap_ctl_cart@addBatchCart' ]); #加入购物车
...
|--------------------------------------------------------------------------
 */
route::group(array('prefix' => 'shopadmin', 'middleware' =>['desktop_middleware_DefenseCSRFByReferer', 'desktop_middleware_CheckRequestXSS'] ), function() {
