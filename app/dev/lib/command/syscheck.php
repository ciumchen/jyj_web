...
class dev_command_syscheck extends base_shell_prototype{
    ...
    public function command_allcheck()
    {
        $args = func_get_args();
    ...
    function database(){
        //检测mysql函数库是否可用
        $rst = function_exists('mysqli_connect') && function_exists('mysqli_get_server_info');
        ...
        if(!$rst){
            ...
            if(!is_null($host)){
                if(!is_null($password)){
                    $rs = mysqli_connect($host, $username, $password);
                }elseif(!is_null($username)){
                    $rs = mysqli_connect($host, $username);
                }else{
                    $rs = mysqli_connect($host);
                }
                $db_ver = mysqli_get_server_info($rs);
            }else{
