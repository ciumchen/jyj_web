...
class image_ctl_admin_manage extends desktop_controller
{
    ...
    public function _views()
    {
        ...
        $sub_menu = array(
            ...
            3=>array('label'=>app::get('image')->_('商家入驻图片'),'optional'=>false,'filter'=>array('target_type'=>['seller'],'disabled'=>0)),
     ...
     */
    public function image_upload_without_tag()
    {
       ...
       $image_src = base_storager::modifier($imageData['url']);
        ...
        $result = array('success'=>true, 'data'=>['url' => $image_src]);
    ...
    public function _set_tag($imageData)
    {
       ...
       $tag_names = explode(' ',$_POST['tag']['name']);
       $domain = 'image';
       foreach($tag_names as $tag_name)
       {
           $tag = app::get('image')->rpcCall('systag.tag.getByName', ['domain'=>$domain, 'tag_name'=>$tag_name]);
           if(empty($tag))
           {
               $tag_id = app::get('image')->rpcCall('systag.tag.save', ['domain'=>$domain, 'tag_name'=>$tag_name]);
           }
           else
           {
               $tag_id = $tag['tag_id'];
           }
           app::get('image')->rpcCall('systag.rel.bind', ['domain'=>$domain, 'tag_ids'=>$tag_id, 'rel_ids'=>$imageData['id']]);
       }
    }
...
EOF;
 ...
 //     $tags = $otag->getList('*',array('tag_type'=>'image'));
    ...
    {
        $tags = app::get('image')->rpcCall('systag.tag.list', ['domain'=>'image','page_no'=>1, 'page_size'=>1000]);
        $formatTag = [];
        foreach($tags['list'] as $tag)
        {
            ...
            $tmp['tag_type'] = 'image';
            $tmp['tag_abbr'] = '';
            $tmp['tag_bgcolor'] = $tag['tag_color'];
            $tmp['tag_fgcolor'] = '';
            $tmp['rel_count'] = $tag['rel_count'];
            $tmp['params'] = null;
    ...
    function image_lib($tag='',$page=1){
        ...
        if($tag > 0)
        {
            $tag_ids = app::get('image')->rpcCall('systag.rel.getRelsByTag',
                [
                    'tag_id'=>$tag,
                    'domain'=>'image',
            ...
            if(count($tag_ids) ==  0) return view::make('image/image_lib.html', $pagedata);
