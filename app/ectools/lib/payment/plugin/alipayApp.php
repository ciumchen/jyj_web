 ...
 */
//require_once(__DIR__.'/alipaysdk/AopSdk.php');
...
final class ectools_payment_plugin_alipayApp extends ectools_payment_app implements ectools_interface_payment_app {
     ...
     * @支付宝固定参数
     */
    public $Service3 = "alipay.trade.refund";    //接口3 统一收单交易退款接口
    public $Service4 = "alipay.trade.fastpay.refund.query";    //接口4 统一收单交易退款查询
    public $_input_charset = 'utf-8';    //字符编码格式
    public $signType = 'RSA2';//签名加密方式
    ...
    public function setting(){
        return array(
            ...
            'app_id'=>array(
                'title'=>app::get('ectools')->_('支付宝应用id'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'mer_id'=>array(
...
/* 以下为退款代码 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ */
    ...
    public function dorefund($payment)
    {
        ...
        $app_id    = trim($this->getConf('app_id', __CLASS__));
        $private_key = trim($this->getConf('private_key', __CLASS__));

        if(!$mer_id || !$mer_key || !$mer_email || !$app_id || !$private_key)
        {
        ...
        $biz_content = [
                ...
                'refund_amount'     => $payment['refund_fee'],
                'refund_reason'     => '协议退款',
                'out_request_no'    => $payment['refund_id'],
           ];
        $this->add_field('app_id',            $app_id);
        $this->add_field('method',            $this->Service3);
        $this->add_field('format',            'JSON');
        $this->add_field('charset',           $this->_input_charset);
        $this->add_field('sign_type',         $this->signType);
        $this->add_field('timestamp',         date('Y-m-d H:i:s',time()));
        $this->add_field('version',           1.0);
        $this->add_field('biz_content',       json_encode($biz_content));
        logger::info('请求alipayApp参数：'.var_export($this->fields,1));
        $aliSignObj = kernel::single('ectools_payment_plugin_alipay_server');
        $sign = $aliSignObj->generateSign($this->fields,$this->signType,$private_key);
        $this->add_field('sign',            $sign);
        ...
        //提交至阿里处理
        $result = json_decode($http->post($url,$this->fields),1);
        logger::info('支付宝返回退款参数：'.var_export($result,1));
        if (!empty($result) && $result['alipay_trade_refund_response']['code'] == '10000') {
            //如果支付宝返回成功  对接查账接口查询退款状态 重新组织post参数
            $queryParam = [
                'trade_no'          => '',
                'out_trade_no'      => $payment['payment_id'],
                'out_request_no'    => $payment['refund_id'],
            ...
            $aliRefundParams = [
                'app_id'            => $app_id,
                'method'            => $this->Service4,
                'format'            => 'JSON',
                'charset'           => $this->_input_charset,
                'sign_type'         => $this->signType,
                'timestamp'         => date('Y-m-d H:i:s',time()),
                'version'           => '1.0',
                'biz_content'       => json_encode($queryParam),
            ];
            $aliRefundParams['sign'] = $aliSignObj->generateSign($aliRefundParams,$this->signType,$private_key);
            //如果该接口返回了查询数据，则代表退款成功，如果没有查询到则代表未退款成功，可以调用退款接口进行重试
            for ($i=0; $i <10 ; $i++) {
                $queryResult = json_decode($http->post($url,$aliRefundParams),1);
                if (!empty($queryResult)) {
                    break;
                }
            }
            logger::info('支付宝查账接口返回参数：'.var_export($queryResult,1));
            if (empty($queryResult) || in_array($queryResult['alipay_trade_fastpay_refund_query_response']['sub_code'],array('ACQ.TRADE_NOT_EXIST','ACQ.INVALID_PARAMETER','ACQ.SYSTEM_ERROR')))
            {
                if($queryResult['alipay_trade_fastpay_refund_query_response']['sub_msg'])
                {
                    $msg = $queryResult['alipay_trade_fastpay_refund_query_response']['sub_msg'];
                }else{
                    $msg = '退款失败，请重试';
                }
                ...
                return [
                  'status'=>$queryResult['alipay_trade_fastpay_refund_query_response']['code'] == '10000' ? 'succ' : 'failed',
    ...
    public function gen_refund_form()
    {
        ...
        //更改退款接口地址
        $refund_submit_url = 'https://openapi.alipay.com/gateway.do';
