    ...
    public function __construct($app = null)
    {
        ...
        if($_GET['ctl']!="passport"&&$_GET['ctl']!=""){
            ...
            if(!$this->status&&$this->status==0){
                ...
                echo view::make('desktop/auth_error.html', $pagedata);exit;
            }
        }
        ###如果不是超级管理员就查询操作权限
        if(!$this->user->is_super()){
            if(!$this->user->chkground($this->workground)){
                ...
                if (request::ajax()){
                    $this->splash('error', null, app::get('desktop')->_("您无权操作"));
                }
        ...
        $perm_id = $obj_model->permissionId($_GET);
        if($perm_id){
            if(!$this->has_permission($perm_id, $this->workground))
            {
                header('Content-Type:text/html; charset=utf-8');
                if (request::ajax()){
                    $this->splash('error', null, app::get('desktop')->_("您无权操作"));
