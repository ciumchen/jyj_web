...
class topapi_api_v1_item_filterItems implements topapi_interface_api{
    ...
    public function handle($params)
    {
        ...
        {
            $area = explode(':', $filterItems['shopInfo']['shop_area']);
            $filterItems['shopInfo']['area'] = $area[1];
            $filterItems['shopInfo']['shop_area'] = $area[0];
            //判断是否设置地区配置 没有则默认为关闭
            if (empty($filterItems['shopInfo']['addr_switch']) || !in_array($filterItems['shopInfo']['addr_switch'],['open','close']))
            {
                $filterItems['shopInfo']['addr_switch'] = 'close';
            }
            $apiParams = [
            'shop_id'    => $filterItems['shopInfo']['shop_id'],
            'page_name'  => 'index',
            'platform'   => 'app',
            ];
            $data = app::get('topshop')->rpcCall('sysdecorate.widgets.get', $apiParams);
            $filterItems['shopInfo']['logo_image'] = base_storager::modifier($data[0]['params']['imgurl'],'');
            $pagedata['shop'] = $filterItems['shopInfo'];
            //获取百度地图ak值
