...
class topdev_ctl_apis extends topdev_controller {
    ...
    public function group()
    {
        ...
        if( $apiType == 'topmanageapi' )
        {
            $list = kernel::single('topdev_apis')->getTopManageApiGroupList(input::get('v','v1'));
            $this->runtimePath[] = ['title' => app::get('topdev')->_('商家端APP聚合API列表')];
            $this->runtimePath[] = ['title' => input::get('v','v1')];
            $this->activeMenu = '商家端APP聚合API';
        }
        elseif( $apiType == 'topapi' )
        {
    ...
    public function info()
    {
        ...
        if( $method && $apis[$method] )
        {
            ...
            elseif( $apiType == 'topmanageapi' )
            {
                $this->runtimePath[] = ['title' => app::get('topdev')->_('商家APP聚合API列表')];
                $this->runtimePath[] = ['title' => $v];
                $groupName = explode('.',$method)[0];
                $url = url::action('topdev_ctl_apis@group', ['apitype'=>$apiType,'group'=>$groupName]);
                $this->activeMenu = '商家APP聚合API';
            }
            else
            {
                $this->runtimePath[] = ['title' => app::get('topdev')->_('系统API列表')];
    ...
    public function testApi()
    {
        ...
        elseif($apiType == 'topmanageapi' )
        {
            $url = kernel::base_url(1).kernel::url_prefix().'/shop/topapi';
            $pagedata['apiParams'] = $apiParams;
            $apiParams['method'] = $method;
        }
        else
        ...
        try{
            $result = client::post($url, ['form_params' => $apiParams])->getBody();
        }catch(Exception $e){
