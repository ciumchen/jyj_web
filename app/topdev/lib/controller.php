...
class topdev_controller extends base_routing_controller {
    ...
    public function getMenu()
    {
        ...
        $devMenu->group(array('group_name' => 'API管理'), function() use ($devMenu) {
            ...
            $devMenu->group(array('group_name' => '商家端APP聚合API','icon'=>'fa fa-th-list'), function() use ($devMenu) {
                $devMenu->group(array('group_name'=>'v1'),  function() use ($devMenu) {
                    $list = kernel::single('topdev_apis')->getTopManageApiGroupList('v1');
                    foreach( $list as $row )
                    {
                        $name = $this->menugroup[$row['name']] ?: $row['name'];
                        $devMenu->add($name, array('topdev_ctl_apis@group',['group'=>$row['name'], 'apitype'=>'topmanageapi', 'v'=>'v1']) ,$row['count']);
                    }
                });
                $devMenu->group(array('group_name'=>'v2'),  function() use ($devMenu) {
                    $list = kernel::single('topdev_apis')->getTopManageApiGroupList('v2');
                    foreach( $list as $row )
                    {
                        $name = $this->menugroup[$row['name']] ?: $row['name'];
                        $devMenu->add($name, array('topdev_ctl_apis@group',['group'=>$row['name'], 'apitype'=>'topmanageapi', 'v'=>'v1']) ,$row['count']);
