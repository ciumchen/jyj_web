...
class topshop_ctl_register extends topshop_controller {
    ...
    public function signPage()
    {
        ...
        //0325版本修改后的代码
        $pagedata['license'] = app::get('sysshop')->getConf('sysshop.register.setting_sysshop_license');
        return $this->page('topshop/register/signPage_0325.html', $pagedata);
    }
  ...
  //}
    ...
    {
        redirect::action('topshop_ctl_register@enterProcessCompanyInfo')->send();exit;
    ...
    public function enterProcessCompanyInfo()
    {
        ...
        // 0325版本修改
        return $this->page('topshop/register/enterProcessCompanyInfo_0325.html', $pagedata);
    }
    ...
    public function enterProcessCompanyInfoAction()
    {
        ...
        foreach ($array as $value) {
            $companyInfo['shop_info'][$value] = empty($companyInfo['shop_info'][$value]) ? [] : $companyInfo['shop_info'][$value];
            $companyInfo['shop_info'][$value] = json_encode($companyInfo['shop_info'][$value]);
        }
        ...
        //数据验证
        $validator = validator::make ([
            'shop_info.bankID' => $companyInfo['shop_info']['bankID'],
            'shop_info.bank_name' => $companyInfo['shop_info']['bank_name'],
            'shop_info.bank_user_name' => $companyInfo['shop_info']['bank_user_name'],
            ], [
            'shop_info.bankID' => 'required',
            'shop_info.bank_name' => 'required',
            'shop_info.bank_user_name' => 'required',
            ], [
            'shop_info.bankID' => app::get('topshop')->_('请输入开户银行的银行卡账号信息'),
            'shop_info.bank_name' => app::get('topshop')->_('请输入详细的开户银行'),
            'shop_info.bank_user_name' => app::get('topshop')->_('请输入银行卡的开户人姓名'),
            ]);
        ...
        // $this->store('economicInfo', $companyInfo);

        //退货人信息
        //数据验证
        $validator = validator::make ([
            'shop_info.return_back_addr' => $companyInfo['shop_info']['return_back_addr'],
            'shop_info.return_back_contacts' => $companyInfo['shop_info']['return_back_contacts'],
            'shop_info.return_back_cmobile' => $companyInfo['shop_info']['return_back_cmobile'],
            ], [
            'shop_info.return_back_addr' => 'required',
            'shop_info.return_back_contacts' => 'required',
            'shop_info.return_back_cmobile' => 'required',
            ], [
            'shop_info.return_back_addr' => app::get('topshop')->_('请填写收货人地址详细到街道'),
            'shop_info.return_back_contacts' => app::get('topshop')->_('请填写收货人姓名'),
            'shop_info.return_back_cmobile' => app::get('topshop')->_('请填写收货人电话'),
            ]);
        try{
            $validator->newFails ();
        }catch(Exception $e) {
        ...
        $this->store('returnBackInfo', $companyInfo);


        //店铺信息配置保存
        return $this->__enterProcessShopInfoAction($companyInfo);
    }
     ...
     */
    private function __enterProcessShopInfoAction($shopInfo)
    {
        ...
        else
        {
            ...
            $shop_info['shopuser_name'] = $companyInfo['shop_info']['representative'];

            $post = $companyInfo;
            $post['shop_name'] = $companyInfo['company_name'];
            $post['shopuser_name'] = $companyInfo['shop_info']['representative'];
            $post['shop'] = $shop;
