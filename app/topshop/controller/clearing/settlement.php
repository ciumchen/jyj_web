...
class topshop_ctl_clearing_settlement extends topshop_controller
{
    ...
    public function detail()
    {
        ...
        if($postSend['timearea'])
        {
            $pagedata['timearea'] = $postSend['timearea'];
            $timeArray = explode('-', $postSend['timearea']);
            $filter['settlement_time_than']  = strtotime($timeArray[0])-86400;
            $filter['settlement_time_lthan'] = strtotime($timeArray[1])+86400;
        }
        else
        {
            $filter['settlement_time_than']  = strtotime("-7 day",strtotime(date('Y-m-d')))-86400;
            $filter['settlement_time_lthan'] = strtotime(date('Y-m-d'))+86400;
