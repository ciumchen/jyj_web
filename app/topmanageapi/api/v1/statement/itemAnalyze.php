 ...
 * topapi
 *
 * -- statement.shop.item.analyze
 * -- 交易数据分析
 *
 * @copyright  Copyright (c) 2005-2016 ShopEx Technologies Inc. (http://www.shopex.cn)
 * @license  http://ecos.shopex.cn/ ShopEx License
 */
class topmanageapi_api_v1_statement_itemAnalyze implements topmanageapi_interface_api{

    /**
     * 接口作用说明
     */
    public $apiDescription = '商品销售分析';
    ...
    public function handle($params)
    {
        ...
        if ($params['itemtime'])
        {
            $postSend['itemtime'] = $params['itemtime'];
        }
        $postSend['pages'] = $params['pages_no'] ? $params['pages_no'] :1;

        $result = kernel::single('topmanageapi_statement_itemAnalyze')->get_itemAnalyze_data($postSend,$type);
        return $result;
        }
