...
class syspromotion_api_activity_register{
    ...
    private function __getActivity($params,$shopdata)
    {
        $nowtime = time();
        $catId = [];
        foreach ($params['register_item'] as $key => $value) {
            $itemParams = [
                'item_id' => $value['item_id'],
                'fields'  => 'cat_id',
            ];
            $itemInfo = app::get('syspromotion')->rpcCall('item.get',$itemParams);
            //获取一级类目id
            $catParams = [
                'cat_id' => $itemInfo['cat_id'],
            ];
            $cat = app::get('syspromotion')->rpcCall('category.cat.get.data',$catParams);
            $catId[] = $cat['lv1']['cat_id'];
        }
