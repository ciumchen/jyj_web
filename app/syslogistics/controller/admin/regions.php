...
class syslogistics_ctl_admin_regions extends desktop_controller {
    ...
    /**
     * [showBaiduAk 添加百度地图api秘钥]
     * @return null
     */
    public function showBaiduAk()
    {
        $pagedata['baiduAk'] = app::get('syslogistics')->getConf('baiDuapiAk');
        return view::make('syslogistics/delivery/area_baidu.html',$pagedata);
    }
     ...
     * @return null
     */
    public function addBaiduAk()
    {
        $this->begin('?app=syslogistics&ctl=admin_regions&act=index');
        $baiduAk = input::get('baiduAk');
        if(!trim($baiduAk))
        {
            $this->end(false, app::get('syslogistics')->_('百度秘钥值不能为空!'));
        }
        app::get('syslogistics')->setConf('baiDuapiAk', $baiduAk);
